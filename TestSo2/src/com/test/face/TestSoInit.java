...
public interface TestSoInit extends Library {
	...
	/**
	 * @brief 语音合成初始化
	 * @param [in]pWorkPath,语音合成工作路径,初始化的时候将从这个路径找配置， 词典，合成库等数据。
	 * @return 错误码
	 * @retval 0成功，非0失败
	 */
	...
	int tsttsDelSession(int pSessionId);
	 ...
	 * @return 该参数需要多少缓冲区存储，返回0表示不存在这样的参数， 如果传入的缓冲区大小小于该值，会被截断
	 */
	// uint32_t tsttsParamGet(uint32_t sessionId, const char* strName, char*
	// pBuf, uint32_t bufSize);
	int tsttsParamGet(int sessionId, String strName, String pBuf, int bufSize);
	 ...
	 * @retval 0成功，非0失败
	 */
	// TSERR_TYPE tsttsParamSet(uint32_t sessionId, const char* strName, const
	// char* pBuf);
	int tsttsParamSet(int pSessionId, String strName, String pBuf);
	 ...
	 * @param [in]textLenByByte,要合成文本的长度，以字节为单位 //这行取消* @param
	 *        [in]textCoding,要合成文本的字符编码
	 *        ，0:自动检测;1:utf-8;2:utf-16;4:utf-32;17:GBK;33:BIG5
	 * @return 错误码
	 * @retval 0成功，非0失败
	 */
	// TSERR_TYPE tsttsPrepareText(uint32_t sessionId, const void* pTextBuf,
	// uint32_t textLenByByte);
	// int tsttsPrepareText(int pSessionId,String pTextBuf,int textLenByByte);
	 ...
	 */
	// TSERR_TYPE tsttsGetAudio(uint32_t sessionId, void* pAudioBuf, uint32_t*
	// pLen);
	// int tsttsGetAudio(int sessionId, void* pAudioBuf, uint32_t* pLen);
	 ...
	 * @retval 0成功，非0失败
	 */
	// TSERR_TYPE tsttsTextToAudioFile(uint32_t sessionId, const char* strFile,
	// const void* pTextBuf, uint32_t textLenByByte);
	int tsttsTextToAudioFile(int sessionId, String strFile, String pTextBuf, int textLenByByte);
