CREATE OR REPLACE PROCEDURE SP_ORDER_ADDORUPDATE_REFUND(v_opttype            in number, --操作类型
                                                        ...
                                                        v_refundaccount      in number, --退款账户
...
BEGIN
  ...
  if v_opttype = 0 then
    ...
    select o.paystatus, o.agentid
      into l_paystatus, l_agentid
      from SALE_ORDERINFO o
     where o.ordersno = v_ordersno;
  
    if l_status != 0 or l_paystatus != 2 then
      v_errorcode := '0007'; --申请记录状态或者订单支付状态不匹配
      v_result    := '1111';
      ROLLBACK;
      return;
    end if;
