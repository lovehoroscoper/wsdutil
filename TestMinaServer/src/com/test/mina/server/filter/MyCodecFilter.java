 ...
 */
public class MyCodecFilter extends IoFilterAdapter {
	/** A logger for this class */
	private static final Logger LOGGER = LoggerFactory.getLogger(MyCodecFilter.class);

	private static final Class<?>[] EMPTY_PARAMS = new Class[0];
	private static final IoBuffer EMPTY_BUFFER = IoBuffer.wrap(new byte[0]);

	private final AttributeKey ENCODER = new AttributeKey(MyCodecFilter.class, "encoder");
	private final AttributeKey DECODER = new AttributeKey(MyCodecFilter.class, "decoder");
	private final AttributeKey DECODER_OUT = new AttributeKey(MyCodecFilter.class, "decoderOut");
	private final AttributeKey ENCODER_OUT = new AttributeKey(MyCodecFilter.class, "encoderOut");
	...
	/**
	 * 
	 * Creates a new instance of MyCodecFilter, associating a factory for
	 * the creation of the encoder and decoder.
	 * 
	 * @param factory
	 *            The associated factory
	 */
	public MyCodecFilter(ProtocolCodecFactory factory) {
		if (factory == null) {
	...
	/**
	 * Creates a new instance of MyCodecFilter, without any factory. The
	 ...
	 */
	public MyCodecFilter(final ProtocolEncoder encoder, final ProtocolDecoder decoder) {
		if (encoder == null) {
	 ...
	 * Creates a new instance of MyCodecFilter, without any factory. The
	 * encoder/decoder factory will be created as an inner class, using the two
	 * parameters (encoder and decoder), which are class names. Instances for
	 * those classes will be created in this constructor.
	 * 
	 * @param encoder
	 *            The class responsible for encoding the message
	 * @param decoder
	 *            The class responsible for decoding the message
	 */
	public MyCodecFilter(final Class<? extends ProtocolEncoder> encoderClass, final Class<? extends ProtocolDecoder> decoderClass) {
	 ...
	 * until the decoder throws an exception.
	 * 
		...
		public WriteFuture flush() {
			...
			while (!bufferQueue.isEmpty()) {
				...
				if (!(encodedMessage instanceof IoBuffer) || ((IoBuffer) encodedMessage).hasRemaining()) {
					...
					nextFilter.filterWrite(session, new EncodedWriteRequest(encodedMessage, future, writeRequest.getDestination()));
				}
			}
	...
	/**
	 * Dispose the encoder, removing its instance from the session's attributes,
	 * and calling the associated dispose method.
	 */
	...
	/**
	 * Dispose the decoder, removing its instance from the session's attributes,
	 * and calling the associated dispose method.
	 */
