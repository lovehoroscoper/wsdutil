 ...
 * 通过定期在通信网络的源端和目的端同时改用新的Key，便能更进一步提高数据的保密性。 初始Key值为64位，但DES算法规定，其中第8、
 * 16、......64位是奇偶校验位，不参与DES运算。故Key 实际可用位数便只有56位。 3-DES（TripleDES）：该算法被用来解决使用 DES
 * 技术的 56 位时密钥日益减弱的强度， 其方法是：使用两个密钥对明文运行 DES 算法三次，从而得到 112 位有效密钥强度。 TripleDES 又称为
 * DESede（表示加密、解密和加密这三个阶段）。 扩展阅读：http://zh.wikipedia.org/zh/3DES *
 */
...
//		String target = decrypt(cryptograph+"1");
		String target = decrypt("PxfhGC/CQt4prV88i2mrSA==");
		// 解密密文
